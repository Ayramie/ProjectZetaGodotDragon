extends Control
class_name AnvilUI
## Anvil UI for forging metal weapons.
...
func start_forging(inv: Inventory) -> void:
	inventory = inv
	visible = true
	state = State.MENU
	_build_recipe_list()
...
func _build_recipe_list() -> void:
	...
	# Get all anvil recipes
	var recipes: Array = RecipeDatabase.get_recipes(RecipeDatabase.RecipeType.ANVIL)
...
func _add_recipe_button(recipe: Dictionary) -> void:
	...
	# Craft button
	var btn := Button.new()
	btn.text = "Forge" if can_craft else "Cannot Forge"
...
func _on_recipe_selected(recipe: Dictionary) -> void:
	...
	craft_timer = 0.0
	craft_time = recipe.get("craft_time", 4.0)

	recipe_panel.visible = false
	crafting_panel.visible = true
	state = State.CRAFTING

	crafting_label.text = "Forging %s..." % recipe.get("name", "item")
...
func _complete_crafting() -> void:
	...
	result_label.text = "Forged %s!" % result_name
	result_panel.visible = true

	AudioManager.play_sound("forge_complete") if AudioManager.has_method("play_sound") else null
